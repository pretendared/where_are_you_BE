...
export class MemberService {
  ...
  async getMembers(boardCode: string) {
    ...
    boardUsers.map(bu => {
      if (bu.role == "MASTER") {
        masters.push({ id: bu.userId, nickname: bu.user.nickname, profileImage: bu.user.profileImage })
      } else {
        members.push({ id: bu.userId, nickname: bu.user.nickname, profileImage: bu.user.profileImage })
      }
    });
